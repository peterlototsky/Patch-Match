...
function output = voteNNF(NNF, source_image, source_alpha)
    global patch_size;
    ...
    source_alpha = padarray(source_alpha,[floor(patch_size/2),floor(patch_size/2)],0,'both');
    
    source_alpha = abs(source_alpha);
    ...
    for i = 1:1:size(NNF,1)
        for j = 1:1:size(NNF,2)
            ...
            output( abs(i - yup):1:i + ydown , abs(j - xleft):1:j + xright,:) = output( abs(i - yup):1:i + ydown , abs(j - xleft):1:j + xright,:) + (source_image( abs(NNF(i,j,1) - yup):1:ydown + NNF(i,j,1), abs(NNF(i,j,2) - xleft):1:NNF(i,j,2) + xright,:) .* source_alpha( abs(NNF(i,j,1) - yup):1:NNF(i,j,1) + ydown , abs(NNF(i,j,2) - xleft):1:NNF(i,j,2) + xright));
            averageArr( abs(i - yup):1:i + ydown , abs(j - xleft):1:j + xright,:) = averageArr( abs(i - yup):1:i + ydown , abs(j - xleft):1:j + xright,:) + source_alpha( abs(NNF(i,j,1) - yup):1:NNF(i,j,1) + ydown , abs(NNF(i,j,2) - xleft):1:NNF(i,j,2) + xright,:);
