...
function NNF = patchMatchNNF(target_image, source_image)
    ...
    rng('default')
     ...
     rng('default')
    NNF(:,:,1) = reshape( randi(size(NNF,1),size(NNF,1)*size(NNF,2),1), size(NNF,2), size(NNF,1)) .';
    NNF(:,:,2) = reshape( randi(size(NNF,2),size(NNF,1)*size(NNF,2),1), size(NNF,2), size(NNF,1)) .';
